...
const Skills = ({ skillData }: Props) => {
    ...
    return (
        <SectionWrapper id='skills' className="min-h-screen mt-12 md:mt-0 mx-4 md:mx-0 xl:my-20 2xl:my-0">
            ...
            <div className="w-full max-w-2xl overflow-x-auto scroll-hide mx-auto mt-6 bg-white dark:bg-grey-800 p-2 flex justify-between items-center gap-3 rounded-md">
                {categories.map((c: string, i: number) => (
                    <span 
                        key={i} 
                        onClick={() => setCategory(c)} 
                        className={`p-1.5 md:p-2 text-sm md:text-base flex-1 text-center cursor-pointer rounded-md whitespace-nowrap ${
